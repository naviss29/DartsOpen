...
export function NextMatchAlert({ boardNumber, match }: Props) {
  ...
  return (
    ...
    >
      <div
        className={`bg-surface-secondary border-2 border-success-solid rounded-2xl p-10 text-center max-w-lg w-full mx-4 shadow-2xl shadow-success-solid/20 transition-all duration-500 ${
          visible ? "scale-100 opacity-100" : "scale-90 opacity-0"
        }`}
      >
        <div className="text-5xl mb-4 animate-bounce">🎯</div>
        <p className="text-success-solid text-sm font-semibold uppercase tracking-widest mb-2">
          Prochain match — Cible {boardNumber}
        </p>
        <h2 className="text-3xl font-bold text-text-primary mb-2">
          {match.player1.player_name}
        </h2>
        <p className="text-text-secondary text-xl mb-2">contre</p>
        <h2 className="text-3xl font-bold text-text-primary mb-6">
          {match.player2.player_name}
        </h2>
        <div className="inline-flex items-center gap-2 rounded-full bg-success-solid/20 border border-success-solid/30 px-4 py-2">
          <span className="w-2 h-2 rounded-full bg-success-solid animate-pulse" />
          <span className="text-success-solid text-sm font-medium">
