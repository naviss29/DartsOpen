  ...
  return (
    <form action={action} className="space-y-6">
      ...
      <Card as="section" className="space-y-4">
        ...
        <FormField label="Nom du tournoi" htmlFor="name" error={state?.errors?.name?.[0]}>
          <Input
            id="name"
            name="name"
            type="text"
        ...
        <div className="grid grid-cols-2 gap-4">
          <FormField label="Date" htmlFor="date" error={state?.errors?.date?.[0]}>
            <Input id="date" name="date" type="date" required min={today} defaultValue={state?.fields?.date} />
          </FormField>
          <FormField label="Lieu" htmlFor="location" error={state?.errors?.location?.[0]}>
      ...
      <Card as="section" className="space-y-4">
        ...
        {quickMode ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-4">
              <FormField label="Nombre de joueurs max" htmlFor="max_players" error={state?.errors?.max_players?.[0]}>
                <Input id="max_players" name="max_players" type="number" min="2" max="512" defaultValue={state?.fields?.max_players ?? "32"} required />
              </FormField>
              <FormField label="Nombre de cibles disponibles" htmlFor="nb_boards" error={state?.errors?.nb_boards?.[0]}>
            ...
            <p className="text-xs text-darts-green-dark">
          ...
          <>
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField label="Droits d'inscription (€ / joueur)" htmlFor="entry_fee" error={state?.errors?.entry_fee?.[0]} hint="Le total facturé = ce montant × nb de joueurs par équipe">
                <Input id="entry_fee" name="entry_fee" type="number" min="0" defaultValue={state?.fields?.entry_fee ?? "10"} required />
              </FormField>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <FormField label="Nombre de poules" htmlFor="nb_pools" error={state?.errors?.nb_pools?.[0]}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField label="Joueurs par équipe" htmlFor="players_per_team" error={state?.errors?.players_per_team?.[0]} hint="Ex : 1 = solo, 2 = doublette">
              ...
              <FormField label="Qualifiés par poule" htmlFor="advancement_per_pool" error={state?.errors?.advancement_per_pool?.[0]} hint="Ex : 8 poules × 2 = 16 finalistes">
            ...
            <div className="space-y-2 rounded-lg border border-gray-200 p-4">
              ...
              <label className="flex cursor-pointer items-start gap-3">
                <input type="radio" name="registration_mode" value="ONLINE" defaultChecked={(state?.fields?.registration_mode ?? "ONLINE") === "ONLINE"} className="mt-0.5 accent-darts-green" />
                <div>
              ...
              <label className="flex cursor-pointer items-start gap-3">
                <input type="radio" name="registration_mode" value="ONSITE" defaultChecked={state?.fields?.registration_mode === "ONSITE"} className="mt-0.5 accent-darts-green" />
                <div>
            ...
            <div className="space-y-2 rounded-lg border border-gray-200 p-4">
              ...
              <label className="flex cursor-pointer items-start gap-3">
                <input type="radio" name="scoring_mode" value="ELECTRONIC" defaultChecked={(state?.fields?.scoring_mode ?? "ELECTRONIC") !== "TRADITIONAL"} className="mt-0.5 accent-darts-green" />
                <div>
              ...
              <label className="flex cursor-pointer items-start gap-3">
                <input type="radio" name="scoring_mode" value="TRADITIONAL" defaultChecked={state?.fields?.scoring_mode === "TRADITIONAL"} className="mt-0.5 accent-darts-green" />
                <div>
